...
함수 선언과 함수 표현식의 가장 큰 차이점은 함수 선언은 호이스팅(Hoisting)이 되지만, 함수 표현식은 적용되지 않음.
...
※ 함수 선언와 달리 중괄호 실행블럭 끝에 세미콜론을 표기. 함수 선언이나 함수 표현식 모두 실제 세미콜론지정 여부와 관계없이 실행되지만 코드 가독성을 위해 구분하는 것을 지향. 
