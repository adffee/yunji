...
- 함수 표현식에 대한 축약 형태로 자바의 람다식과 표현식 규칙이 동일하되 자바의 '->' 가 아닌 '=>'를 사용하는 차이점.
또한, 자바의 경우 람다식을 참조하기 위한 함수형 인터페이스가 필요한 반면 js에서는 동적 타입 결정으로 인해  별도의 함수형 인터페이스에 정의된 추상 메서드 리턴타입과 형식인수 정의등이 필요치 않아 표현식에 대한 let, const, vat등의 구문 변수로 바로 참조 가능.
...
- 자바에서 익명 이너 객체나 람다식이 일회성 동작에 대한 단축 문법인것 처럼 js에서도 일회성으로 사용되는 것이 아닌, 이처럼 반복적으로 동일한 호출을 해야하는 경우 중복
호출 정의가 되므로 상기 예시(53 ~ 63행)와 같이 74행의 identifyOddEven함수 표현식내에서 외부에 미리 정의된 함수를 직접 호출하는 것이 효율적.
*/
