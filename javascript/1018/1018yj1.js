...
참조하는 것은 이벤트를 지정한 요소 자체의 참조가 아닌 요소가 기본적으로 가지고 있는 활성 이벤트 객체의 참조가 할당.

===============================================================================

    < target 프로포티 >

- 임의 요소가 기본적으로 가지고 있는 활성 이벤트 객체에 할당되어 있는 프로퍼티로써 활성 이벤트 객체의 주체가 되는 요소의 참조를 반환. 즉, 람다식 내에서 사용 불가한 this 를 대체하여 익명함수에서의 this 와 동일한 참조를 부여 가능.
*/
...
- 요소에 대한 포인터 이벤트 대상 여부를 지정. pointerEvents 지정을 하지 않는다 해도 요소가 기본적으로 가지고 있는 클릭에 대한 시각적 이미지 효과만 적용되지 않을뿐, addEventListener 메서드를 통해 모든 요소에 대한 이벤트 동작 부여 가능.
단, pointerEvents 속성 값 할당에 따라 요소가 기본적으로 가지고 있는 포인터 이벤트 효과를 지정 또는 해제할 뿐만 아니라, addEventListener 메서드를 통해 요소에 부여한 이벤트 동작 자체도 지정 또는 해제 가능.
...
- 마우스 포인터가 객체 위로 이동하거나 벗어 났을 때 이벤트 핸들러 할당은 mouseenter, mouseleave 와 동일. 단, mouseenter, mouseleave 는 인벤트 핸들러를 할당한 해당 객체에만 적용되는 반면, mouseover, mouseout 해당 객체의 자식 요소에도 이벤트 핸들러 할당이 상속되어 적용되는 특성 차이.
*/
...
단, 여기서 기본 이벤트 동작이라 함은 클릭과 같은 행위를 했을 때의 이벤트 효과나 클릭 자체의 동작이 아닌 클릭으로 인해 인과되는 결과 동작을 의미함에 주의. 
요소 자체에 대한 것이 아닌, 요소가 기본적으로 가지고 있는 이벤트 동작에 대한 지정이므로 this 로는 불가하며 익명함수나 람다식에 임의 매개변수를 지정함으로써 요소가 가지고 있는 기본적인 활성 이벤트 객체의 참조를 통해 호출 가능.
*/
